package com.nuaa.larry.wifiindoorlocation.widge;
...
public class WaterWaveView extends View {
	...
	private int mCircleColor = R.color.colorPrimary;
	private int mRingColor = R.color.colorPrimary;
	private int mWaveColor = R.color.white;
	...
	private void init(Context context) {
		...
		flowPaint.setTextSize(UtilBox.sp2px(mContext, 20));

		leftPaint = new Paint();
		leftPaint.setColor(Color.WHITE);
		leftPaint.setStyle(Paint.Style.FILL);
		leftPaint.setAntiAlias(true);
		leftPaint.setTextSize(UtilBox.sp2px(mContext, 20));

		mWavePaint = new Paint();
		mWavePaint.setStrokeWidth(1.0F);
		mWavePaint.setColor(getResources().getColor(mWaveColor));
...
//		};
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(mScreenWidth / 2, mScreenHeight / 2,
				mScreenWidth / 2 - mRingSTROKEWidth, mCirclePaint);
		...
		canvas.drawLine(
				(float)(mScreenWidth * 2.5 / 8),
				(float)(mScreenHeight * 5.5 / 8),
				(float)(mScreenWidth * 5.5 / 8),
				(float)(mScreenHeight * 5.5 / 8),
                linePaint);

		float num = flowPaint.measureText(flowNum);
		canvas.drawText(flowNum,
				mScreenWidth * 4 / 8 - num / 2,
				(float)(mScreenHeight * 5 / 8),
				flowPaint);

		float left = leftPaint.measureText(flowLeft);
		canvas.drawText(flowLeft,
				mScreenWidth * 4 / 8 - left / 2,
				(float)(mScreenHeight * 3 / 8),
				leftPaint);
